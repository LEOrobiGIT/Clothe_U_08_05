...
$(document).ready(function() {
  // Definisci la funzione per l'ordinamento
  function ordinaProdotti(crescente) {
    // Trova la lista di prodotti all'interno della div "row"
    var listaProdotti = $('.row').find('.card');

    // Ordina la lista di prodotti in base al prezzo usando la funzione sort di jQuery
    listaProdotti.sort(function(a, b) {
      var prezzoA = parseFloat($(a).find('.peso').text().replace('$', ''));
      var prezzoB = parseFloat($(b).find('.peso').text().replace('$', ''));
      if (crescente) {
    ...
    // Aggiungi i prodotti in ordine alla div "row"
    $.each(listaProdotti, function(index, item) {
      $('.row').append(item);
    });
  }

  // Aggiungi l'evento al menu dropdown
